...
export default function SummaryTab({ positions, overviewData }) {
                      ...
                      <tr key={s.skill} style={{ borderBottom: `1px solid ${C.slate100}` }}>
                        ...
                        <td style={{ padding: "10px", color: C.slate500, fontSize: 11, verticalAlign: "top" }}>
                          <div style={{ display: "flex", flexWrap: "wrap", gap: 5 }}>
                            {s.reqs.slice(0, 8).map((r) => (
                              <span key={r.id} title={`${r.role} · ${r.client}`} style={{ padding: "2px 8px", background: C.slate100, borderRadius: 3, fontSize: 10, color: C.slate600 }}>
                                {r.id}
                              </span>
                            ))}
                            {s.reqs.length > 8 && <span style={{ fontSize: 10, color: C.slate400, alignSelf: "center" }}>+{s.reqs.length - 8} more</span>}
                          </div>
                        </td>
                      </tr>
                    ))}
