...
def process_dashboard(reqs: Iterable) -> Dict[str, Any]:
    ...
    donut = [
        {
            "name": b["bucket"].replace(" d", " days").replace("Pre-appr.", "Pre-approved"),
            "value": b["value"],
            "color": b["color"],
        }
        for b in aging_pipeline
    ]
...
# ═════════════════════════════════════════════════════════════
# 3. BENCH RESOURCES (MSD rows filtered by bench-IDs)
# ═════════════════════════════════════════════════════════════
def build_bench_resources(
    msd_rows: Iterable,
    bench_ids: Iterable[str],
) -> List[Dict[str, Any]]:
    bench_set = {str(i).strip() for i in bench_ids}
    emp_map: Dict[str, Dict[str, Any]] = {}

    for r in msd_rows:
        g = (lambda k: r.get(k)) if isinstance(r, dict) else (lambda k: getattr(r, k, None))
        emp_id = (g("employee_id") or "").strip()
        if not emp_id or emp_id not in bench_set:
            continue
        ...
        if is_bench_proj and bench_days is not None:
            emp_map[emp_id]["benchDays"] = bench_days
            emp_map[emp_id]["benchProject"] = project

        row_skills = parse_skills(g("skillsets"), g("l4_sub_skill"), g("l3_skill_family"))
        if len(row_skills) > len(emp_map[emp_id]["skills"]):
            emp_map[emp_id]["skills"] = row_skills
